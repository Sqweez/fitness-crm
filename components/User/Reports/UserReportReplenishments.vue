      ...
      <v-expansion-panel>
        <v-expansion-panel-header color="blue darken-2">
          <h5 class="text-white"><b>
            История фин. операций (пополнения)
          </b></h5>
...
export default {
  ...
  computed: {
    ...
    totalCash () {
      return this.items.filter(i => i.payment_type === __hardcoded(1)).reduce((a, c) => {
        return a + c.amount;
      }, 0)
    },
    totalCashless () {
      return this.items.filter(i => i.payment_type === __hardcoded(2)).reduce((a, c) => {
        return a + c.amount;
      }, 0)
    }
  },
