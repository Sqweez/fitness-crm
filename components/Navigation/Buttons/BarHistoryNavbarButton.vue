<template>
<!--  <div title="История бара">
    <button
      @click="$router.push('/economy/history/bar')"
      ref="trigger"
  ...
  >
    <div v-show="dropdownOpen" class="origin-top-right z-10 absolute top-full min-w-44 bg-white border border-gray-200 py-1.5 rounded shadow-lg overflow-hidden mt-1"  :class="align === 'right' ? 'right-0' : 'left-0'">
      ...
      >
        <li>
          <router-link class="font-medium text-sm text-indigo-500 hover:text-indigo-600 flex items-center py-1 px-3" to="/economy/history/bar" @click="dropdownOpen = false">
            <span>История продаж</span>
          </router-link>
        </li>
        <li v-if="$CAN_SALE_BAR">
